...
/** This provides an event sourced behavior. It has a state,
  * [[HelloWorldState]], which stores what the greeting should be (eg, "Hello").
  *
  * Event sourced entities are interacted with by sending them commands. This
  * aggregate supports two commands, a [[UseGreetingMessage]] command, which is
  * used to change the greeting, and a [[Hello]] command, which is a read only
  ...
  * Each event will have an event handler registered for it, and an event
  * handler simply applies an event to the current state. This will be done when
  * the event is first created, and it will also be done when the aggregate is
...
object HelloWorldBehavior {
  ...
  /** Given a sharding [[EntityContext]] this function produces an Akka
    * [[Behavior]] for the aggregate.
    */
  def create(
...
case class HelloWorldState(message: String, timestamp: String) {
  ...
  private def updateMessage(newMessage: String): HelloWorldState =
    copy(newMessage, LocalDateTime.now().toString)
}

object HelloWorldState {

  /** The initial state. This is used if there is no snapshotted state to be
    * found.
    */
  def initial: HelloWorldState =
    HelloWorldState("Hello", LocalDateTime.now.toString)

  /** The [[EventSourcedBehavior]] instances (aka Aggregates) run on sharded
    * actors inside the Akka Cluster. When sharding actors and distributing them
    * across the cluster, each aggregate is namespaced under a typekey that
    * specifies a name and also the type of the commands that sharded actor can
    * receive.
    */
  val typeKey: EntityTypeKey[HelloWorldCommand] =
    EntityTypeKey[HelloWorldCommand]("HelloWorldAggregate")

  /** Format for the hello state.
    *
    * Persisted entities get snapshotted every configured number of events. This
    * means the state gets stored to the database, so that when the aggregate
    * gets loaded, you don't need to replay all the events, just the ones since
    * the snapshot. Hence, a JSON format needs to be declared so that it can be
...
object HelloWorldEvent {
  val Tag: AggregateEventTag[HelloWorldEvent] =
    AggregateEventTag[HelloWorldEvent]
}
...
object GreetingMessageChanged {
  ...
  /** Format for the greeting message changed event.
    *
    * Events get stored and loaded from the database, hence a JSON format needs
    * to be declared so that they can be serialized and deserialized.
    */
  implicit val format: Format[GreetingMessageChanged] = Json.format
}

/** This is a marker trait for commands. We will serialize them using Akka's
  * Jackson support that is able to deal with the replyTo field. (see
  * application.conf)
  */
trait HelloWorldCommandSerializable

/** This interface defines all the commands that the HelloWorldAggregate
  * supports.
  */
